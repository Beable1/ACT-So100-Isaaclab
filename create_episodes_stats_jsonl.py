...
def _detect_video_chunk_dir(dataset_root: str) -> Tuple[str, str]:
    ...
    for c in candidates:
        ...
        for leaf in ['observation.images.front', 'observation.images.jaw']:
...
def generate_episodes_stats_jsonl(dataset_root: str) -> str:
    ...
    for i, file in enumerate(parquet_files):
        ...
        # For Jia-63ep, we have two cameras: observation.images.front and observation.images.jaw
        episode_basename = file.rsplit('.', 1)[0]  # episode_00001
        cam_keys = ['observation.images.front', 'observation.images.jaw']
        for cam_key in cam_keys:
            cam_dir = os.path.join(video_chunk_root, cam_key)
            video_path = os.path.join(cam_dir, f"{episode_basename}.mp4")
            if os.path.isfile(video_path):
                temp_dir = os.path.join(cam_dir, 'temp_imgs')
                img_paths = _extract_frames_to_temp(video_path, temp_dir)
                if img_paths:
                    ep_ft_array = sample_images(img_paths)
                    # axis=(0,2,3): stats over N,H,W; keep channel dim
                    temp_video_stats = get_feature_stats_img(ep_ft_array, axis=(0, 2, 3), keepdims=True)
                    video_stats = {
                        k: (v if k == "count" else np.squeeze(v / 255.0, axis=0))
                        for k, v in temp_video_stats.items()
                    }
                    video_stats = {k: v.tolist() for k, v in video_stats.items()}
                    # Store under correct key, e.g. "observation.images.front"
                    episode_dic['stats'][cam_key] = video_stats
                else:
                    print(f"[episodes_stats] Warning: no frames extracted from {video_path}")
                shutil.rmtree(temp_dir, ignore_errors=True)
            else:
                print(f"[episodes_stats] Video not found for {cam_key} {episode_basename}; skipping stats for this camera")
        # observation.state
        if 'observation.state' in df.columns:
            # Column is a 1D array of length 6 stored in each row
            # Use position-based access to avoid index issues
            try:
                ...
                print(f"[episodes_stats] Warning: failed to compute observation.state stats for {file}: {e}")
        # action
        if 'action' in df.columns:
            ...
            try:
                ...
                print(f"[episodes_stats] Warning: failed to compute action stats for {file}: {e}")
        # Scalar numeric columns (Jia-63ep schema: see parquet columns)
        # NOTE: Jia-63ep does not have an 'index' column, so we don't require it.
        for col in ['episode_index', 'frame_index', 'timestamp', 'next.reward', 'next.done', 'next.success', 'task_index']:
...
if __name__ == '__main__':
    dataset_root = os.environ.get('DATASET_ROOT', os.path.join(os.environ['HOME'], '.cache/huggingface/lerobot/Beable/Jia-63ep'))
    try:
