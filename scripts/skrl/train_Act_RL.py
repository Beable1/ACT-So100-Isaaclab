...
"""Script to train ACT policy with online RL (PPO) for Isaac Lab environment."""

import argparse
from isaaclab.app import AppLauncher

# add argparse arguments
parser = argparse.ArgumentParser(description="Train ACT policy with online RL for Isaac Lab environment.")
...
parser.add_argument("--checkpoint_dir", type=str, default="outputs/act_rl_checkpoints", help="Directory to save checkpoints.")
parser.add_argument("--resume", type=str, default=None, help="Resume from checkpoint path.")
parser.add_argument("--pretrained", type=str, default=None, help="Load pretrained weights (e.g., from LeRobot) before training.")
...
parser.add_argument("--chunk_size", type=int, default=10, help="Action chunk size for ACT policy.")
parser.add_argument("--hidden_dim", type=int, default=256, help="Hidden dimension for ACT policy.")
...
class ACTPolicy(nn.Module):
    """
    Wrapper for LeRobot ACT policy or custom implementation.
    If 'pretrained_path' is provided, it loads the LeRobot model.
    Otherwise, it uses the custom simple implementation.
    """
    ...
    def __init__(
        ...
        # Try loading LeRobot policy if path provided
        if pretrained_path and HAVE_LEROBOT:
            try:
                print(f"[INFO] Loading LeRobot policy from {pretrained_path}")
                ...
                print(f"[INFO] LeRobot expects input keys: {self.lerobot_input_keys}")
                
                # Load policy
                # Use dataset root for stats if available
                ds_meta = None
                if dataset_root:
                    try:
                ...
                if ds_meta is None:
                    ...
                    try:
                        ...
                        # Create a dummy metadata object that mimics the structure expected by make_policy
                        class DummyMeta:
                            stats = {}
                            task = "dummy_task"
                        
                        # Populate stats with expected keys (using generic values)
                        ...
                        for key, feature in input_features.items():
                            shape = feature.shape
                            # For images: mean=[0.5,0.5,0.5], std=[0.5,0.5,0.5] (approx)
                            if 'image' in key:
                                stats[key] = {
                        ...
                        ds_meta = DummyMeta()
                        ds_meta.stats = stats  # Assign populated stats

                self.lerobot_policy = make_lerobot_policy(cfg=train_cfg.policy, ds_meta=ds_meta)
                self.lerobot_policy.train()  # Set to train mode
                print("[INFO] Successfully loaded LeRobot policy!")
                return
            except Exception as e:
                print(f"[ERROR] Failed to load LeRobot policy: {e}")
                print("[INFO] Falling back to custom ACT implementation...")
        ...
        # Action decoder - outputs chunk_size actions
        ...
        # Log std for action distribution - kept for PPO loss computation only (not used for action sampling)
        # Actions are DETERMINISTIC - no noise added during execution
        self.log_std = nn.Parameter(torch.ones(action_dim) * -5.0)  # Very small for numerical stability
    ...
    def encode(self, state: torch.Tensor, image: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Encode state and optional image into latent representation."""
        # Encode state
        state_feat = self.state_encoder(state)  # (B, hidden_dim)
        
        # Encode image if available
        if self.use_vision and image is not None:
            vision_feat = self.vision_encoder(image).flatten(1)  # (B, 128)
            feat = torch.cat([state_feat, vision_feat], dim=1)  # (B, hidden_dim + 128)
        else:
            feat = state_feat
        
        # Fuse features
        feat = self.fusion(feat)  # (B, hidden_dim)
        return feat
    
    def forward(self, state: torch.Tensor, image: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass to generate action chunk.
        Returns: (action_chunk, log_std)
            action_chunk: (B, chunk_size, action_dim)
            log_std: (action_dim,)
        """
        # If LeRobot policy is loaded, use it
        if self.lerobot_policy is not None:
            # Prepare batch for LeRobot
            batch = {
                "observation.state": state,
            }
            if image is not None:
                # Dynamically map images based on expected keys
                # Expected keys from train_cfg.policy.input_features:
                #   ['observation.images.front', 'observation.images.jaw', 'observation.state']
                # We have concatenated image: main (0-3) + jaw (3-6)
                main_key = next(
                    (k for k in self.lerobot_input_keys if "front" in k or "rgb" in k or "main" in k),
                    "observation.images.front",
                )
                batch[main_key] = image[:, :3]

                if image.shape[1] > 3:
                    # Second camera (jaw)
                    jaw_key = next(
                        (k for k in self.lerobot_input_keys if "jaw" in k and k != main_key),
                        "observation.images.jaw",
                    )
                    batch[jaw_key] = image[:, 3:]

            # Get action from LeRobot policy
            act = self.lerobot_policy.select_action(batch)

            # Normalize action shape to (B, chunk_size, action_dim)
            if isinstance(act, dict):
                # Some policies may return dict; try common keys
                act = act.get("action", next(iter(act.values())))

            act = torch.as_tensor(act, device=state.device, dtype=torch.float32)
            ...
            if act.dim() == 1:
                ...
                # (B, T, A) -> assume already chunked
                pass
            else:
                raise ValueError(f"Unsupported LeRobot action shape: {act.shape}")

            return act, self.log_std

        # Custom Implementation
        # Encode inputs
        feat = self.encode(state, image)  # (B, hidden_dim)

        # Add sequence dimension for transformer
        feat = feat.unsqueeze(1)  # (B, 1, hidden_dim)

        # Transform
        feat = self.transformer(feat)  # (B, 1, hidden_dim)
        feat = feat.squeeze(1)  # (B, hidden_dim)

        # Decode actions
        actions_flat = self.action_decoder(feat)  # (B, chunk_size * action_dim)
        actions = actions_flat.view(-1, self.chunk_size, self.action_dim)  # (B, chunk_size, action_dim)

        # Clamp log_std to keep noise low
    ...
    def get_action(self, state: torch.Tensor, image: Optional[torch.Tensor] = None, deterministic: bool = True) -> torch.Tensor:
        """Get action from policy - ALWAYS DETERMINISTIC (no noise)."""
        actions, log_std = self.forward(state, image)
        # Always return deterministic action (pretrained model output only)
        return actions[:, 0, :]
...
def train_ppo_epoch(
    ...
    """Perform one PPO update epoch with vision support."""
    ...
    for start in range(0, total_samples, batch_size):
        ...
        action_chunk, log_std = policy(batch_states, batch_images)
        action_pred = action_chunk[:, 0, :]  # First action in chunk
        
        # Compute log probability with numerical stability
        # Clamp log_std to prevent nan values
        ...
        try:
            ...
            if batch_images is not None:
                del batch_images
            del new_log_probs, entropy, new_values, ratio, surr1, surr2, policy_loss, value_loss, loss
        except Exception:
...
def main():
    ...
    policy = ACTPolicy(
        ...
        dataset_root=args_cli.dataset_root,  # Need to add this arg to parser first
        device=args_cli.device,
    ).to(device)
    ...
    # Create optimizers
    optimizer_policy = optim.Adam(policy.parameters(), lr=args_cli.learning_rate)
    optimizer_value = optim.Adam(value_net.parameters(), lr=args_cli.learning_rate)
    
    # Create buffer with vision support
    # Note: storing 2 cameras separately, each (3, 512, 512)
    buffer = RolloutBuffer(
        ...
        device=device,
        use_vision=args_cli.use_vision,
        image_channels=6,  # Will concatenate 2 cameras: main(3) + jaw(3) = 6 channels
        image_size=args_cli.image_size,
    )
    ...
    print("\n" + "="*80)
    print("🚀 STARTING PPO TRAINING WITH ACT POLICY")
    print("="*80)
    print(f"\n📋 CONFIGURATION:")
    ...
    print(f"\n🎬 ACT POLICY:")
    print(f"   Chunk Size: {args_cli.chunk_size}")
    ...
    for iteration in range(start_iter, args_cli.max_iterations):
        ...
        for step in range(args_cli.horizon):
            ...
            with torch.no_grad():
                action_chunk, log_std = policy(state, image)  # Pass image!
                action = action_chunk[:, 0, :]  # Take first action in chunk
                ...
                # Get value
                value = value_net(state, image)  # Pass image to value net too!
                    ...
                    if payload_prim.IsValid():
                        ...
                        if previous_jaw_payload_distance is not None:
                            ...
                            if distance_delta > 0:
                                ...
                                # Moving away -> penalty
                                distance_penalty = distance_delta * args_cli.jaw_distance_penalty_scale  # Will be negative
                                reward = reward + torch.tensor([distance_penalty], device=device, dtype=torch.float32).expand_as(reward)
                                if abs(distance_penalty) > 0.01:  # Only print significant penalties
                                    print(f"📍 [RETREATING] Iter {iteration+1} Step {step}: Δdist={distance_delta:.4f}m (away) -> {distance_penalty:.4f} penalty")
            ...
            image_to_store = image[:1] if image is not None and image.shape[0] > 1 else image
            
            # Store transition (with image if vision is enabled)
        ...
        for epoch in range(args_cli.ppo_epochs):
            metrics = train_ppo_epoch(
                ...
                images=images,  # Pass images to PPO update
            )
